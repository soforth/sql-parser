 ...
 */
%{
// Copyright 2014 gosql-parser Author. All Rights Reserved.
//
...
column_list: NAME { emit("COLUMN %s", $1);  $$ = 1; }
  | column_list ',' NAME  { emit("COLUMN %s", $3);  $$ = $1 + 1; }
  ;

select_opts:                          { $$ = 0; }
| select_opts ALL                 { if($$ & 01 != 0) {yyerror("duplicate ALL option")}; $$ = $1 | 01; }
| select_opts DISTINCT            { if($$ & 02 != 0) {yyerror("duplicate DISTINCT option")}; $$ = $1 | 02; }
| select_opts DISTINCTROW         { if($$ & 04 != 0) {yyerror("duplicate DISTINCTROW option")}; $$ = $1 | 04; }
| select_opts HIGH_PRIORITY       { if($$ & 010 != 0) {yyerror("duplicate HIGH_PRIORITY option")}; $$ = $1 | 010; }
| select_opts STRAIGHT_JOIN       { if($$ & 020 != 0) {yyerror("duplicate STRAIGHT_JOIN option")}; $$ = $1 | 020; }
| select_opts SQL_SMALL_RESULT    { if($$ & 040 != 0) {yyerror("duplicate SQL_SMALL_RESULT option")}; $$ = $1 | 040; }
| select_opts SQL_BIG_RESULT      { if($$ & 0100 != 0) {yyerror("duplicate SQL_BIG_RESULT option")}; $$ = $1 | 0100; }
| select_opts SQL_CALC_FOUND_ROWS { if($$ & 0200 != 0) {yyerror("duplicate SQL_CALC_FOUND_ROWS option")}; $$ = $1 | 0200; }
    ;
...
table_factor:
    NAME opt_as_alias index_hint { emit("TABLE %s", $1);  }
...
index_list: NAME  { emit("INDEX %s", $1);  $$ = 1; }
   | index_list ',' NAME { emit("INDEX %s", $3);  $$ = $1 + 1; }
   ;
...
delete_list: NAME opt_dot_star { emit("TABLE %s", $1);  $$ = 1; }
...
stmt: insert_stmt { fmt.Println("STMT"); }
   ;

insert_stmt: INSERT insert_opts opt_into NAME
     opt_col_names
     VALUES insert_vals_list
     opt_ondupupdate { fmt.Printf("INSERTVALS %d %d %s\n", $2, $7, $4);  }
   ;
...
insert_stmt: INSERT insert_opts opt_into NAME opt_col_names
    select_stmt
    opt_ondupupdate { emit("INSERTSELECT %d %s", $2, $4);  }
  ;
...
replace_stmt: REPLACE insert_opts opt_into NAME
     opt_col_names
     VALUES insert_vals_list
     opt_ondupupdate { emit("REPLACEVALS %d %d %s", $2, $7, $4);  }
   ;
...
replace_stmt: REPLACE insert_opts opt_into NAME opt_col_names
    select_stmt
    opt_ondupupdate { emit("REPLACESELECT %d %s", $2, $4);  }
  ;
...
create_database_stmt: 
     CREATE DATABASE opt_if_not_exists NAME { emit("CREATEDATABASE %d %s", $3, $4);  }
   | CREATE SCHEMA opt_if_not_exists NAME { emit("CREATEDATABASE %d %s", $3, $4);  }
   ;

opt_if_not_exists:  /* nil */ { $$ = 0; }
   | IF EXISTS           { if($2 == 0){yyerror("IF EXISTS doesn't exist")}; $$ = $2; /* NOT EXISTS hack */ }
   ;
...
create_table_stmt: CREATE opt_temporary TABLE opt_if_not_exists NAME
   '(' create_col_list ')' { emit("CREATE %d %d %d %s", $2, $4, $7, $5);  }
   ;
...
create_table_stmt: CREATE opt_temporary TABLE opt_if_not_exists NAME
   '(' create_col_list ')'
create_select_statement { emit("CREATESELECT %d %d %d %s", $2, $4, $7, $5);  }
    ;

create_table_stmt: CREATE opt_temporary TABLE opt_if_not_exists NAME
   create_select_statement { emit("CREATESELECT %d %d 0 %s", $2, $4, $5);  }
    ;
...
column_atts: /* nil */ { $$ = 0; }
    ...
    | column_atts NULLX
    | column_atts DEFAULT STRING        { emit("ATTR DEFAULT STRING %s", $3);  $$ = $1 + 1; }
    ...
    | column_atts COMMENT STRING { emit("ATTR COMMENT %s", $3);  $$ = $1 + 1; }
    ;
...
opt_csc: /* nil */
   | opt_csc CHAR SET STRING { emit("COLCHARSET %s", $4);  }
   | opt_csc COLLATE STRING { emit("COLCOLLATE %s", $3);  }
   ;
...
enum_list: STRING { emit("ENUMVAL %s", $1);  $$ = 1; }
   | enum_list ',' STRING { emit("ENUMVAL %s", $3);  $$ = $1 + 1; }
   ;
...
set_expr:
      USERVAR COMPARISON expr { if ($2 != 4) {yyerror("bad set to @%s", $1)};
		 emit("SET %s", $1);  }
    | USERVAR ASSIGN expr { emit("SET %s", $1);  }
    ;
...
expr: expr '+' expr { emit("ADD"); }
   ...
   | expr SHIFT expr { if($2==1){emit("SHIFT left")}else{emit("SHIFT right")} }
   | NOT expr { emit("NOT"); }
   | '!' expr { emit("NOT"); }
...
expr: expr IN '(' val_list ')'       { emit("ISIN %d", $4); }
   ...
   | EXISTS '(' select_stmt ')'      { emit("EXISTS"); if($1 !=0){emit("NOT")}; }
   ;
